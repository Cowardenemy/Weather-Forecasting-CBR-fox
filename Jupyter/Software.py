    ...
    def explain(self):
        ...
        if (self.method == "CCI"):
            ...
            self.pearsonCorrelation = np.array(
                (
                    [np.corrcoef(self.windows[currentWindow, :, currentComponent],
                                 self.targetWindow[:, currentComponent])[
                         0][1]
                     for currentWindow in range(len(self.windows)) for currentComponent in
                     range(self.componentsLen)])).reshape(
                -1,
                self.componentsLen)
            print("Calculando distancia Euclidiana")
            self.euclideanDistance = np.array(
                ([np.linalg.norm(
                    self.targetWindow[:, currentComponent] - self.windows[currentWindow, :, currentComponent])
                  for currentWindow in range(self.windowsLen) for currentComponent in
                  range(self.componentsLen)])).reshape(
                -1,
                self.componentsLen)
            ...
            d = {'index': dict(self.bestSorted).keys(), 'CCI': dict(self.bestSorted).values(), "MAE": self.bestMAE,
                 'index.1': dict(self.worstSorted).keys(), 'CCI.1': dict(self.worstSorted).values(),
                 "MAE.1": self.worstMAE}
